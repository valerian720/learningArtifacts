...
with open("input.txt") as inp:
	# обработка
	dish_count = int(inp.readline()[:-1])
	for i in range(dish_count):
		cur_dish = inp.readline()[:-1].split()
		...
		for j in range(int(cur_dish[2])):
			...
			cur_ingridient = inp.readline()[:-1].split()
			cur_w_type = cur_ingridient[2]
			cur_name = cur_ingridient[0]
			cur_ingr_amount = int(cur_ingridient[1]) * (1000 if cur_w_type == "kg" else 1) * (1000 if cur_w_type == "l" else 1) * (10 if cur_w_type == "tens" else 1)
	...
	check_ingr_count = int(inp.readline()[:-1])
	for i in range (check_ingr_count):
		cur_line = inp.readline()[:-1].split()
		cur_w_type = cur_line[3]
		amount_per_pac = int(cur_line[2]) * (1000 if cur_w_type == "kg" else 1) * (1000 if cur_w_type == "l" else 1) * (10 if cur_w_type == "tens" else 1)
	...
	for i in range (check_ingr_count):
		...
		if (cur_line[0] in ingridient_list):
			...
			amount_per_pac = int(cur_line[1]) * (1000 if cur_w_type == "kg" else 1) * (1000 if cur_w_type == "l" else 1) * (10 if cur_w_type == "tens" else 1)
			# a b c d
